<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
    <div class="h-48 bg-gray-200 relative">
      <!-- Multiple images: show first image with count badge -->
      <img 
        v-if="coverImage" 
        :src="coverImage" 
        :alt="item.title" 
      ...
      <!-- Image count badge -->
      <div v-if="imageCount > 1" class="absolute bottom-3 left-3">
        <span class="px-2 py-1 text-xs font-semibold bg-black/60 text-white rounded-md backdrop-blur-sm">
          📷 {{ imageCount }} photos
        </span>
      </div>
...
// Support both old `imageUrl` (string) and new `imageUrls` (array)
